 ...
 * E2E Browser Tests
 * 
 * These tests verify that WebInfer.js works correctly in a browser environment.
...
describeIf('Browser E2E Tests', () => {
  describe('Global API', () => {
    it('should expose WebInfer global', () => {
      // @ts-ignore
      expect(window.WebInfer).toBeDefined();
    });

    it('should have pipeline function', () => {
      // @ts-ignore
      expect(typeof window.WebInfer.pipeline).toBe('function');
    });

    it('should have tensor function', () => {
      // @ts-ignore
      expect(typeof window.WebInfer.tensor).toBe('function');
    });
  });
  ...
  describe('Tensor Operations in Browser', () => {
    it('should create tensors', () => {
      // @ts-ignore
      const tensor = window.WebInfer.tensor([1, 2, 3, 4], [2, 2]);
      expect(tensor.shape).toEqual([2, 2]);
    });

    it('should perform math operations', () => {
      // @ts-ignore
      const a = window.WebInfer.tensor([1, 2], [2]);
      // @ts-ignore
      const b = window.WebInfer.tensor([3, 4], [2]);
    ...
    it('should compute softmax', () => {
      // @ts-ignore
      const tensor = window.WebInfer.tensor([1, 2, 3], [3]);
  ...
  describe('Runtime Detection', () => {
    it('should detect available runtimes', async () => {
      // @ts-ignore
      const capabilities = await window.WebInfer.detectCapabilities();
  ...
  describe('Memory Management', () => {
    it('should track memory usage', () => {
      // @ts-ignore
      const stats = window.WebInfer.getMemoryStats();
  ...
  describe('IndexedDB Caching', () => {
    it('should cache models in IndexedDB', async () => {
      // @ts-ignore
      const stats = await window.WebInfer.getModelCacheStats();
    ...
    it('should check if model is cached', async () => {
      // @ts-ignore
      const isCached = await window.WebInfer.isModelCached('https://example.com/model.onnx');
...
describe.skip('Model Loading E2E', () => {
  ...
  it('should load model from HuggingFace', async () => {
    // @ts-ignore
    const model = await window.WebInfer.loadModel(MODEL_URL);
  ...
  it('should run inference', async () => {
    // @ts-ignore
    const pipeline = await window.WebInfer.pipeline('text-classification');
