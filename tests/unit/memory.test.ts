...
describe('MemoryManager', () => {
  ...
  describe('Memory Tracking', () => {
    ...
    it('should track allocated memory', () => {
      const tensor = new WebInferTensor(new Array(1000).fill(0), [1000]);
      memoryManager.track(tensor);
    ...
    it('should track peak memory', () => {
      const tensors: WebInferTensor[] = [];
      
      // Allocate multiple tensors
      for (let i = 0; i < 5; i++) {
        const tensor = new WebInferTensor(new Array(1000).fill(i), [1000]);
        memoryManager.track(tensor);
        tensors.push(tensor);
      }
  ...
  describe('Dispose All', () => {
    it('should dispose all tracked resources', () => {
      const tensors = [
        new WebInferTensor([1], [1]),
        new WebInferTensor([2], [1]),
        new WebInferTensor([3], [1]),
      ];
