...
Pipeline 是使用 WebInfer.js 的主要方式。每个 Pipeline 封装了：
...
```typescript
import { tensor, WebInferTensor } from 'WebInferjs';

// 创建 1D 张量
const t1 = new WebInferTensor([1, 2, 3, 4], [4]);

// 创建 2D 张量
const t2 = new WebInferTensor([1, 2, 3, 4], [2, 2]);

// 支持的数据类型
const float32 = new WebInferTensor([1.5, 2.5], [2], 'float32');
const int64 = new WebInferTensor([1, 2], [2], 'int64');
const uint8 = new WebInferTensor([0, 255], [2], 'uint8');
```
...
```typescript
import { getScheduler, TaskPriority } from 'WebInferjs';
...
```typescript
import { getMemoryManager } from 'WebInferjs';
...
```typescript
import { isModelCached, getCachedModel, clearModelCache } from 'WebInferjs';
...
```typescript
import { Tokenizer } from 'WebInferjs';
