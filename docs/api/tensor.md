...
```typescript
import { WebInferTensor } from 'WebInferjs';

// 创建 1D 张量
const t1 = new WebInferTensor([1, 2, 3, 4], [4]);

// 创建 2D 张量
const t2 = new WebInferTensor([1, 2, 3, 4, 5, 6], [2, 3]);

// 指定数据类型
const int64Tensor = new WebInferTensor([1, 2, 3], [3], 'int64');
```
...
```typescript
const tensor = new WebInferTensor([1, 2, 3, 4], [2, 2]);
tensor.get(0, 0); // 1
tensor.get(1, 1); // 4
```
...
```typescript
reshape(newShape: number[]): WebInferTensor
```

```typescript
const t = new WebInferTensor([1, 2, 3, 4, 5, 6], [2, 3]);
...
```typescript
const t = new WebInferTensor([1, 2, 3, 4], [2, 2]);
