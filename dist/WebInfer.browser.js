...
function getTypedArrayConstructor(dtype) {
  switch (dtype) {
    ...
    case "int8":
      return Int8Array;
    default:
      throw new WebInferError(`Unsupported data type: ${dtype}`, ErrorCodes.INVALID_ARGUMENT, { dtype });
  }
}
...
function validateShape(shape) {
  for (let i = 0; i < shape.length; i++) {
    ...
    if (dim === void 0 || !Number.isInteger(dim) || dim < 0) {
      throw new WebInferError(`Invalid shape dimension at index ${i}: ${dim}`, ErrorCodes.INVALID_ARGUMENT, { shape, index: i, dimension: dim });
    }
  }
}
...
function tensor(data, shape, dtype = "float32") {
  if (Array.isArray(data) && data.length > 0 && Array.isArray(data[0])) {
    ...
    for (const row of data) {
      if (row.length !== cols) {
        throw new WebInferError("Nested arrays must have consistent dimensions", ErrorCodes.INVALID_ARGUMENT);
      }
      flatData.push(...row);
    }
    return new WebInferTensor(flatData, shape ?? [rows, cols], dtype);
  }
  const inferredShape = shape ?? [data.length];
  return new WebInferTensor(data, inferredShape, dtype);
}
function zeros(shape, dtype = "float32") {
  const size = calculateSize(shape);
  const TypedArrayCtor = getTypedArrayConstructor(dtype);
  const data = new TypedArrayCtor(size);
  return new WebInferTensor(data, shape, dtype);
}
...
function arange(start, stop, step = 1, dtype = "float32") {
  ...
  return new WebInferTensor(data, [size], dtype);
}
...
function eye(n, dtype = "float32") {
  ...
  return new WebInferTensor(data, [n, n], dtype);
}
function add(a, b) {
  if (typeof b === "number") {
    ...
    return new WebInferTensor(result2, a.shape, a.dtype);
  }
  if (a.size !== b.size) {
    throw new WebInferError("Tensor sizes must match for element-wise operations", ErrorCodes.TENSOR_SHAPE_MISMATCH, { aShape: a.shape, bShape: b.shape });
  }
  ...
  return new WebInferTensor(result, a.shape, a.dtype);
}
function sub(a, b) {
  if (typeof b === "number") {
    const result2 = new Float32Array(a.size);
...
function matmul(a, b) {
  if (a.shape.length !== 2 || b.shape.length !== 2) {
    throw new WebInferError("matmul requires 2D tensors", ErrorCodes.INVALID_ARGUMENT, { aShape: a.shape, bShape: b.shape });
  }
  const [m, k1] = a.shape;
  const [k2, n] = b.shape;
  if (k1 !== k2) {
    throw new WebInferError(`Matrix dimensions incompatible for multiplication: (${m}x${k1}) @ (${k2}x${n})`, ErrorCodes.TENSOR_SHAPE_MISMATCH, { aShape: a.shape, bShape: b.shape });
  }
  ...
  return new WebInferTensor(result, [m, n], a.dtype);
}
function softmax(t, axis = -1) {
  ...
  if (actualAxis < 0 || actualAxis >= t.shape.length) {
    throw new WebInferError(`Invalid axis ${axis} for tensor with ${t.shape.length} dimensions`, ErrorCodes.INVALID_ARGUMENT, { axis, shape: t.shape });
  }
  if (t.shape.length === 1) {
    let max = -Infinity;
    ...
    return new WebInferTensor(result, t.shape, t.dtype);
  }
  ...
  throw new WebInferError("Softmax currently only supports 1D tensors or 2D tensors along the last axis", ErrorCodes.NOT_IMPLEMENTED, { shape: t.shape, axis });
}
function relu(t) {
  const data = t.toFloat32Array();
  const result = new Float32Array(t.size);
...
function sum(t, axis) {
  ...
  if (t.shape.length === 2) {
    ...
    if (actualAxis === 0) {
      ...
      return new WebInferTensor(result, [cols], t.dtype);
    } else {
      ...
      return new WebInferTensor(result, [rows], t.dtype);
    }
  }
  throw new WebInferError("Sum along axis currently only supports up to 2D tensors", ErrorCodes.NOT_IMPLEMENTED, { shape: t.shape, axis });
}
...
function argmax(t, axis) {
  ...
  if (t.shape.length === 2 && actualAxis === 1) {
    ...
    return new WebInferTensor(result, [rows], "int32");
  }
  throw new WebInferError("Argmax along axis currently only supports 2D tensors along the last axis", ErrorCodes.NOT_IMPLEMENTED, { shape: t.shape, axis });
}
function concat(tensors, axis = 0) {
  if (tensors.length === 0) {
    throw new WebInferError("Cannot concatenate empty array of tensors", ErrorCodes.INVALID_ARGUMENT);
  }
  if (tensors.length === 1) {
  ...
  if (!first) {
    throw new WebInferError("First tensor is undefined", ErrorCodes.INVALID_ARGUMENT);
  }
  ...
  for (let i = 1; i < tensors.length; i++) {
    ...
    if (t.shape.length !== first.shape.length) {
      throw new WebInferError("All tensors must have the same number of dimensions", ErrorCodes.TENSOR_SHAPE_MISMATCH);
    }
    for (let j = 0; j < first.shape.length; j++) {
      if (j !== actualAxis && first.shape[j] !== t.shape[j]) {
        throw new WebInferError(`Shape mismatch at dimension ${j}`, ErrorCodes.TENSOR_SHAPE_MISMATCH);
      }
    }
  }
  ...
  if (first.shape.length === 1) {
    ...
    return new WebInferTensor(result, newShape, first.dtype);
  }
...
var Task = class {
  ...
  cancel() {
    if (this._status === "pending") {
      ...
      const cancelError = new WebInferError("Task was cancelled", ErrorCodes.INFERENCE_CANCELLED, { taskId: this.id });
  ...
  wait() {
    ...
    if (this._status === "cancelled") {
      return Promise.reject(new WebInferError("Task was cancelled", ErrorCodes.INFERENCE_CANCELLED, { taskId: this.id }));
    }
    return new Promise((resolve, reject) => {
...
var InferenceScheduler = class {
  ...
  schedule(modelId, executor, priority = "normal") {
    if (this.disposed) {
      throw new WebInferError("Scheduler has been disposed", ErrorCodes.RUNTIME_NOT_INITIALIZED);
    }
    ...
    const timeoutExecutor = () => {
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          reject(new WebInferError(`Task timed out after ${timeout}ms`, ErrorCodes.INFERENCE_TIMEOUT, { timeout }));
        }, timeout);
...
var WebGPURuntime = class {
  ...
  async initialize() {
    if (this.initialized)
      return;
    if (!navigator.gpu) {
      throw new WebInferError("WebGPU is not supported in this browser", ErrorCodes.RUNTIME_NOT_AVAILABLE);
    }
    this.adapter = await navigator.gpu.requestAdapter({
      powerPreference: "high-performance"
    });
    if (!this.adapter) {
      throw new WebInferError("Failed to get WebGPU adapter", ErrorCodes.RUNTIME_INIT_FAILED);
    }
  ...
  async executeModel(inputs, metadata) {
    ...
    for (const outputSpec of metadata.outputs) {
      ...
      outputs.push(new WebInferTensor(outputData, outputSpec.shape, "float32"));
  ...
  ensureInitialized() {
    if (!this.initialized || !this.device) {
      throw new WebInferError("WebGPU runtime is not initialized", ErrorCodes.RUNTIME_NOT_INITIALIZED);
    }
  }
  /**
   * Dispose the runtime
...
var WebNNRuntime = class {
  ...
  async initialize() {
    if (this.initialized)
      return;
    if (!navigator.ml) {
      throw new WebInferError("WebNN is not supported in this browser", ErrorCodes.RUNTIME_NOT_AVAILABLE);
    }
    try {
      ...
      try {
        ...
        throw new WebInferError(`Failed to create WebNN context: ${error instanceof Error ? error.message : String(error)}`, ErrorCodes.RUNTIME_INIT_FAILED);
      }
    }
    this.initialized = true;
  }
  ...
  ensureInitialized() {
    if (!this.initialized || !this.context) {
      throw new WebInferError("WebNN runtime is not initialized", ErrorCodes.RUNTIME_NOT_INITIALIZED);
    }
  }
  /**
   * Get device type
...
var WASMRuntime = class {
    ...
    for (const outputSpec of metadata.outputs) {
      ...
      if (inputs.length > 0 && inputs[0]) {
        ...
        if (outputSpec.name.includes("logits") || outputSpec.name.includes("class")) {
          ...
          outputTensor = new WebInferTensor(outputData, outputSpec.shape, "float32");
        }
      } else {
        outputTensor = new WebInferTensor(new Float32Array(outputSize), outputSpec.shape, "float32");
      }
  ...
  ensureInitialized() {
    if (!this.initialized || !this.module) {
      throw new WebInferError("WASM runtime is not initialized", ErrorCodes.RUNTIME_NOT_INITIALIZED);
    }
  }
  /**
...
var Cache = class {
  constructor(options = {}) {
    ...
    this.options = {
      ...
      name: options.name ?? "WebInfer-cache"
    };
...
var ModelDownloadCache = class {
  constructor(cacheName = "WebInfer-models") {
...
var Tokenizer = class _Tokenizer {
  constructor() {
    ...
    __publicField(this, "padTokenId");
    __publicField(this, "unkTokenId");
  ...
  static async fromJSON(json) {
    ...
    if (padFromVocab !== void 0) {
      tokenizer.padTokenId ?? (tokenizer.padTokenId = padFromVocab);
    }
    const unkFromVocab = inferFromVocab("[UNK]");
    if (unkFromVocab !== void 0) {
      tokenizer.unkTokenId ?? (tokenizer.unkTokenId = unkFromVocab);
    }
    ...
    if (data.added_tokens) {
      for (const token of data.added_tokens) {
        ...
        if (content.includes("pad"))
          tokenizer.padTokenId ?? (tokenizer.padTokenId = token.id);
        if (content.includes("unk"))
          tokenizer.unkTokenId ?? (tokenizer.unkTokenId = token.id);
  ...
  getConfig() {
    return {
      ...
      padTokenId: this.padTokenId ?? 0,
      unkTokenId: this.unkTokenId ?? 0,
      clsTokenId: this.clsTokenId,
      sepTokenId: this.sepTokenId,
      maskTokenId: this.maskTokenId,
      bosTokenId: this.bosTokenId,
...
var TextClassificationPipeline = class extends BasePipeline {
  ...
  async preprocess(input) {
    ...
    const inputIds = new WebInferTensor(new Float32Array(encoded.inputIds), [1, encoded.inputIds.length], "float32");
    const attentionMask = new WebInferTensor(new Float32Array(encoded.attentionMask), [1, encoded.attentionMask.length], "float32");
  ...
  async runInference(inputs) {
    ...
    return [new WebInferTensor(logits, [1, numClasses], "float32")];
  }
  /**
...
var FeatureExtractionPipeline = class extends BasePipeline {
  ...
  async runInference(inputs) {
    ...
    return [new WebInferTensor(embeddings, [1, seqLen, this.embeddingDim], "float32")];
  }
  /**
   * Postprocess model outputs
   */
...
var ImagePreprocessor = class _ImagePreprocessor {
  ...
  async processBatch(inputs) {
    ...
    if (!firstTensor) {
      return new WebInferTensor(new Float32Array(0), [0], "float32");
    }
    ...
    return new WebInferTensor(batchData, [batchSize, channels, height, width], "float32");
  }
  /**
...
var AudioPreprocessor = class _AudioPreprocessor {
  ...
  async processRaw(input) {
    ...
    return new WebInferTensor(audioData, [1, audioData.length], "float32");
  }
  /**
   * Load audio from URL
   */
  ...
  computeMelSpectrogram(audio) {
    ...
    if (numFrames <= 0) {
      return new WebInferTensor(new Float32Array(nMels), [1, nMels], "float32");
    }
    ...
    return new WebInferTensor(melSpec, [numFrames, nMels], "float32");
  }
  /**
   * Dispose resources
   */
...
async function downloadJson(modelId, filename, options = {}) {
  ...
  const response = await fetchWithAuth(url, options.token);
  if (!response.ok) {
    throw new WebInferError(`Failed to download ${filename} from ${modelId}: ${response.status}`, ErrorCodes.MODEL_NOT_FOUND);
  }
...
async function downloadModel(modelId, options = {}) {
  ...
  if (!modelFile) {
    throw new WebInferError(`No ONNX model found in ${modelId}. Please ensure the model has an ONNX file.`, ErrorCodes.MODEL_NOT_FOUND, { modelId, triedFiles: ONNX_MODEL_FILES });
  }
...
var TokenClassificationPipeline = class _TokenClassificationPipeline extends BasePipeline {
   ...
   * This lets apps reuse models loaded via WebInfer.loadModel(url).
   */
  ...
  async run(input, options) {
    ...
    const inputIds = new WebInferTensor(encoded.inputIds, [1, encoded.inputIds.length], "int64");
    const attentionMask = new WebInferTensor(encoded.attentionMask, [1, encoded.attentionMask.length], "int64");
    const tokenTypeIds = new WebInferTensor(encoded.tokenTypeIds ?? new Array(encoded.inputIds.length).fill(0), [1, encoded.inputIds.length], "int64");
...
var ObjectDetectionPipeline = class extends BasePipeline {
  ...
  async preprocess(input) {
    ...
    if (inputs.length === 1) {
      ...
      return [new WebInferTensor(tensor2.toFloat32Array(), [1, ...tensor2.shape], "float32")];
    }
...
var AutomaticSpeechRecognitionPipeline = class extends BasePipeline {
  ...
  async preprocess(input) {
    ...
    if (tensors.length === 1) {
      const t = tensors[0];
      return [new WebInferTensor(t.toFloat32Array(), [1, ...t.shape], "float32")];
    }
    return tensors;
  }
  /**
...
var QuestionAnsweringPipeline = class extends BasePipeline {
  ...
  async preprocess(input) {
    ...
    return [
      new WebInferTensor(BigInt64Array.from(encoded.inputIds.map((id) => BigInt(id))), [1, encoded.inputIds.length], "int64"),
      new WebInferTensor(BigInt64Array.from(encoded.attentionMask.map((m) => BigInt(m))), [1, encoded.attentionMask.length], "int64")
    ];
  }
  /**
   * Postprocess model output
  ...
  async postprocess(outputs, _options) {
    ...
    const startProbs = softmax(new WebInferTensor(startLogits, [seqLen], "float32")).toFloat32Array();
    const endProbs = softmax(new WebInferTensor(endLogits, [seqLen], "float32")).toFloat32Array();
    let bestStart = 0;
    let bestEnd = 0;
    let bestScore = 0;
...
function quantizeTensor(tensor2, type, options = {}) {
  ...
  const zpValue = params.zeroPoint instanceof Int32Array ? Array.from(params.zeroPoint) : params.zeroPoint;
  return {
    tensor: new WebInferTensor(Array.from(quantizedData), shape, dtype),
    scale: scaleValue,
    zeroPoint: zpValue
  };
}
...
async function pruneModel(modelData, options = {}) {
  ...
  for (const weight of weights) {
    totalParams += weight.data.length;
    const tensor2 = new WebInferTensor(Array.from(weight.data), weight.shape, "float32");
...
var WebInferDebugger = class {
  ...
  defaultLogger(level, message, data) {
    ...
    const prefix = `[WebInfer.js ${timestamp}] [${level.toUpperCase()}]`;
    switch (level) {
      case "debug":
...
function getDebugger(config) {
  if (!globalDebugger || config) {
    globalDebugger = new WebInferDebugger(config);
  }
...
<head>
  ...
  <title>WebInfer.js Performance Dashboard</title>
  <style>
    :root {
      --bg-primary: #0d1117;
      --bg-secondary: #161b22;
  ...
  <div class="dashboard">
    <header>
      <h1>
        ...
        WebInfer.js Performance Dashboard
      </h1>
      <div class="status">
    ...
    <footer>
      Generated at ${(/* @__PURE__ */ new Date()).toLocaleString()} | WebInfer.js Performance Monitor
    </footer>
  </div>
</body>
</html>
...
function generateAsciiDashboard(monitor) {
  ...
  const lines = [
    "\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557",
    "\u2551             WebInfer.js Performance Monitor Dashboard                   \u2551",
    "\u2560\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2563",
    "\u2551                                                                          \u2551",
...
//# sourceMappingURL=WebInfer.browser.js.map
