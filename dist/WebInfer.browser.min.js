var Ot=Object.defineProperty;var Yn=(o,t,e)=>t in o?Ot(o,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):o[t]=e;var Zn=(o,t)=>()=>(o&&(t=o(o=0)),t);var es=(o,t)=>{for(var e in t)Ot(o,e,{get:t[e],enumerable:!0})};var f=(o,t,e)=>(Yn(o,typeof t!="symbol"?t+"":t,e),e);var rn={};es(rn,{cancelPreload:()=>mt,clearModelCache:()=>ht,deleteCachedModel:()=>dt,getCachedModel:()=>ut,getModelCacheStats:()=>ft,getPreloadStatus:()=>pt,getPreloadedModel:()=>gt,isModelCached:()=>me,loadModelData:()=>H,preloadModel:()=>ct,preloadModels:()=>lt});async function ms(o){try{let t=await fetch(o,{method:"HEAD"}),e=t.headers.get("Accept-Ranges"),n=t.headers.get("Content-Length"),s=t.headers.get("ETag")??void 0;return{supports:e==="bytes",size:n?parseInt(n,10):0,etag:s}}catch{return{supports:!1,size:0}}}async function gs(o,t,e,n){let s=new AbortController,i=setTimeout(()=>s.abort(),n);try{let r=await fetch(o,{headers:{Range:`bytes=${t}-${e}`},signal:s.signal});if(r.status!==206&&r.status!==200)throw new Error(`HTTP ${r.status}: ${r.statusText}`);return await r.arrayBuffer()}finally{clearTimeout(i)}}async function ys(o,t){let{chunkSize:e=5*1024*1024,parallelConnections:n=4,timeout:s=3e4,onProgress:i}=t,{supports:r,size:a,etag:c}=await ms(o);if(!r||a<e*2)return sn(o,s,i);let l=await L.getDownloadState(o);if(!l||c&&l.totalSize!==a){let S=Math.ceil(a/e),P=[];for(let E=0;E<S;E++){let z=E*e,I=Math.min(z+e-1,a-1);P.push({index:E,start:z,end:I,downloaded:!1})}l={url:o,totalSize:a,downloadedSize:0,chunks:P,startedAt:Date.now()},await L.deleteModel(o)}let u=l.chunks.filter(S=>!S.downloaded),d=l.downloadedSize,p=Date.now(),m=d,y=()=>{if(!i)return;let S=Date.now(),P=(S-p)/1e3,E=d-m,z=P>0?E/P:0,I=a-d,tt=z>0?I/z*1e3:0;i({loaded:d,total:a,percent:d/a*100,speed:z,eta:tt,currentChunk:l.chunks.filter(Nt=>Nt.downloaded).length,totalChunks:l.chunks.length}),p=S,m=d},w=[...u],k=new Map;for(;w.length>0||k.size>0;){for(;w.length>0&&k.size<n;){let S=w.shift(),P=(async()=>{try{let E=await gs(o,S.start,S.end,s);await L.saveChunk(o,S.index,E),S.downloaded=!0,d+=E.byteLength,l.downloadedSize=d,await L.saveDownloadState(l),y()}finally{k.delete(S.index)}})();k.set(S.index,P)}k.size>0&&await Promise.race(k.values())}let b=await L.getChunks(o),T=new Uint8Array(a),A=0;for(let S of b)T.set(new Uint8Array(S),A),A+=S.byteLength;return await L.saveMeta({url:o,size:a,etag:c,cachedAt:Date.now(),chunks:b.length,complete:!0}),await L.deleteDownloadState(o),T.buffer}async function sn(o,t,e){let n=new AbortController,s=setTimeout(()=>n.abort(),t);try{let i=await fetch(o,{signal:n.signal});if(!i.ok)throw new Error(`HTTP ${i.status}: ${i.statusText}`);let r=i.headers.get("Content-Length"),a=r?parseInt(r,10):0;if(!i.body||!e||a===0)return await i.arrayBuffer();let c=i.body.getReader(),l=[],u=0,d=Date.now();for(;;){let{done:m,value:y}=await c.read();if(m)break;l.push(y),u+=y.length;let w=(Date.now()-d)/1e3,k=w>0?u/w:0,b=a-u,T=k>0?b/k*1e3:0;e({loaded:u,total:a,percent:u/a*100,speed:k,eta:T})}let h=new Uint8Array(u),p=0;for(let m of l)h.set(m,p),p+=m.length;return h.buffer}finally{clearTimeout(s)}}async function H(o,t={}){let{cache:e=!0,forceDownload:n=!1,resumable:s=!0}=t;if(e&&!n){let r=await L.getModel(o);if(r)return console.log(`\u2713 Model loaded from cache: ${o}`),t.onProgress?.({loaded:r.byteLength,total:r.byteLength,percent:100,speed:0,eta:0}),r}let i;return s?i=await ys(o,t):i=await sn(o,t.timeout??3e4,t.onProgress),e&&(s||(await L.saveChunk(o,0,i),await L.saveMeta({url:o,size:i.byteLength,cachedAt:Date.now(),chunks:1,complete:!0}))),i}function ct(o,t={}){return pe.preload(o,t)}function lt(o,t={}){return Promise.all(o.map(({url:e,priority:n})=>pe.preload(e,{...t,priority:n})))}async function me(o){return(await L.getMeta(o))?.complete??!1}async function ut(o){return L.getModel(o)}async function dt(o){return L.deleteModel(o)}async function ht(){return L.clear()}async function ft(){return L.getStats()}function pt(o){return pe.getStatus(o)}function mt(o){pe.cancel(o)}async function gt(o){return pe.get(o)}var ps,N,B,U,it,L,at,pe,Le=Zn(()=>{"use strict";ps="WebInfer-model-cache",N="meta",B="chunks",U="download-state",it=class{constructor(){f(this,"db",null);f(this,"dbPromise",null)}async openDB(){return this.db?this.db:this.dbPromise?this.dbPromise:(this.dbPromise=new Promise((t,e)=>{let n=indexedDB.open(ps,1);n.onupgradeneeded=s=>{let i=s.target.result;i.objectStoreNames.contains(N)||i.createObjectStore(N,{keyPath:"url"}),i.objectStoreNames.contains(B)||i.createObjectStore(B,{keyPath:["url","index"]}).createIndex("url","url",{unique:!1}),i.objectStoreNames.contains(U)||i.createObjectStore(U,{keyPath:"url"})},n.onsuccess=()=>{this.db=n.result,t(this.db)},n.onerror=()=>e(n.error)}),this.dbPromise)}async getMeta(t){let e=await this.openDB();return new Promise((n,s)=>{let a=e.transaction(N,"readonly").objectStore(N).get(t);a.onsuccess=()=>n(a.result??null),a.onerror=()=>s(a.error)})}async saveMeta(t){let e=await this.openDB();return new Promise((n,s)=>{let i=e.transaction(N,"readwrite");i.objectStore(N).put(t),i.oncomplete=()=>n(),i.onerror=()=>s(i.error)})}async saveChunk(t,e,n){let s=await this.openDB();return new Promise((i,r)=>{let a=s.transaction(B,"readwrite");a.objectStore(B).put({url:t,index:e,data:n}),a.oncomplete=()=>i(),a.onerror=()=>r(a.error)})}async getChunks(t){let e=await this.openDB();return new Promise((n,s)=>{let c=e.transaction(B,"readonly").objectStore(B).index("url").getAll(t);c.onsuccess=()=>{let l=c.result;l.sort((u,d)=>u.index-d.index),n(l.map(u=>u.data))},c.onerror=()=>s(c.error)})}async getModel(t){let e=await this.getMeta(t);if(!e||!e.complete)return null;let n=await this.getChunks(t);if(n.length===0)return null;let s=n.reduce((a,c)=>a+c.byteLength,0),i=new Uint8Array(s),r=0;for(let a of n)i.set(new Uint8Array(a),r),r+=a.byteLength;return i.buffer}async saveDownloadState(t){let e=await this.openDB();return new Promise((n,s)=>{let i=e.transaction(U,"readwrite");i.objectStore(U).put(t),i.oncomplete=()=>n(),i.onerror=()=>s(i.error)})}async getDownloadState(t){let e=await this.openDB();return new Promise((n,s)=>{let a=e.transaction(U,"readonly").objectStore(U).get(t);a.onsuccess=()=>n(a.result??null),a.onerror=()=>s(a.error)})}async deleteDownloadState(t){let e=await this.openDB();return new Promise((n,s)=>{let i=e.transaction(U,"readwrite");i.objectStore(U).delete(t),i.oncomplete=()=>n(),i.onerror=()=>s(i.error)})}async deleteModel(t){let e=await this.openDB();await new Promise((s,i)=>{let r=e.transaction(N,"readwrite");r.objectStore(N).delete(t),r.oncomplete=()=>s(),r.onerror=()=>i(r.error)}),(await this.getChunks(t)).length>0&&await new Promise((s,i)=>{let r=e.transaction(B,"readwrite"),l=r.objectStore(B).index("url").openCursor(IDBKeyRange.only(t));l.onsuccess=u=>{let d=u.target.result;d&&(d.delete(),d.continue())},r.oncomplete=()=>s(),r.onerror=()=>i(r.error)}),await this.deleteDownloadState(t)}async clear(){let t=await this.openDB(),e=[N,B,U];for(let n of e)await new Promise((s,i)=>{let r=t.transaction(n,"readwrite");r.objectStore(n).clear(),r.oncomplete=()=>s(),r.onerror=()=>i(r.error)})}async getStats(){let t=await this.openDB();return new Promise((e,n)=>{let r=t.transaction(N,"readonly").objectStore(N).getAll();r.onsuccess=()=>{let a=r.result;e({models:a.filter(c=>c.complete).length,totalSize:a.reduce((c,l)=>c+(l.complete?l.size:0),0)})},r.onerror=()=>n(r.error)})}},L=new it;at=class{constructor(){f(this,"tasks",new Map);f(this,"queue",[]);f(this,"maxConcurrent",2);f(this,"activeCount",0)}preload(t,e={}){let n=this.tasks.get(t);if(n)return n.promise;let s,i,r=new Promise((l,u)=>{s=l,i=u}),a={url:t,priority:e.priority??0,options:e,promise:r,resolve:s,reject:i,status:"pending"};this.tasks.set(t,a);let c=this.queue.findIndex(l=>{let u=this.tasks.get(l);return u&&u.priority<a.priority});return c===-1?this.queue.push(t):this.queue.splice(c,0,t),this.processQueue(),r}async processQueue(){for(;this.queue.length>0&&this.activeCount<this.maxConcurrent;){let t=this.queue.shift();if(!t)break;let e=this.tasks.get(t);!e||e.status!=="pending"||(this.activeCount++,e.status="loading",this.downloadTask(e).finally(()=>{this.activeCount--,this.processQueue()}))}}async downloadTask(t){try{let e=await H(t.url,t.options);t.status="complete",t.resolve(e)}catch(e){t.status="error",t.reject(e instanceof Error?e:new Error(String(e)))}}isPreloaded(t){return this.tasks.get(t)?.status==="complete"}getStatus(t){return this.tasks.get(t)?.status??"not_found"}async get(t){let e=this.tasks.get(t);return e&&(e.status==="complete"||e.status==="loading")?e.promise:null}cancel(t){let e=this.tasks.get(t);e&&e.status==="pending"&&(this.tasks.delete(t),this.queue=this.queue.filter(n=>n!==t),e.reject(new Error("Preload cancelled")))}clear(){for(let[,t]of this.tasks)t.status==="pending"&&t.reject(new Error("Preload cleared"));this.tasks.clear(),this.queue=[]}},pe=new at});var x=class extends Error{constructor(e,n,s){super(e);f(this,"code");f(this,"details");this.code=n,this.details=s,this.name="WebInferError"}},M={RUNTIME_NOT_AVAILABLE:"RUNTIME_NOT_AVAILABLE",RUNTIME_INIT_FAILED:"RUNTIME_INIT_FAILED",RUNTIME_NOT_INITIALIZED:"RUNTIME_NOT_INITIALIZED",MODEL_NOT_FOUND:"MODEL_NOT_FOUND",MODEL_LOAD_FAILED:"MODEL_LOAD_FAILED",MODEL_INVALID_FORMAT:"MODEL_INVALID_FORMAT",MODEL_NOT_LOADED:"MODEL_NOT_LOADED",INFERENCE_FAILED:"INFERENCE_FAILED",INFERENCE_TIMEOUT:"INFERENCE_TIMEOUT",INFERENCE_CANCELLED:"INFERENCE_CANCELLED",OUT_OF_MEMORY:"OUT_OF_MEMORY",MEMORY_LEAK_DETECTED:"MEMORY_LEAK_DETECTED",TENSOR_SHAPE_MISMATCH:"TENSOR_SHAPE_MISMATCH",TENSOR_DTYPE_MISMATCH:"TENSOR_DTYPE_MISMATCH",TENSOR_DISPOSED:"TENSOR_DISPOSED",PIPELINE_NOT_SUPPORTED:"PIPELINE_NOT_SUPPORTED",PIPELINE_INPUT_INVALID:"PIPELINE_INPUT_INVALID",INVALID_ARGUMENT:"INVALID_ARGUMENT",NOT_IMPLEMENTED:"NOT_IMPLEMENTED",UNKNOWN_ERROR:"UNKNOWN_ERROR"};var ts=0;function ns(){return`tensor_${++ts}_${Date.now().toString(36)}`}function Ee(o){switch(o){case"float32":return Float32Array;case"float16":return Float32Array;case"int32":return Int32Array;case"int64":return BigInt64Array;case"uint8":case"bool":return Uint8Array;case"int8":return Int8Array;default:throw new x(`Unsupported data type: ${o}`,M.INVALID_ARGUMENT,{dtype:o})}}function ee(o){return o.length===0?1:o.reduce((t,e)=>t*e,1)}function ss(o){for(let t=0;t<o.length;t++){let e=o[t];if(e===void 0||!Number.isInteger(e)||e<0)throw new x(`Invalid shape dimension at index ${t}: ${e}`,M.INVALID_ARGUMENT,{shape:o,index:t,dimension:e})}}var g=class o{constructor(t,e,n="float32"){f(this,"id");f(this,"dtype");f(this,"shape");f(this,"size");f(this,"_data");f(this,"_isDisposed",!1);ss(e),this.id=ns(),this.dtype=n,this.shape=Object.freeze([...e]),this.size=ee(this.shape);let s=this.size;if(t.length!==s)throw new x(`Data length (${t.length}) does not match shape ${JSON.stringify(e)} (expected ${s})`,M.TENSOR_SHAPE_MISMATCH,{dataLength:t.length,expectedSize:s,shape:e});if(t instanceof Array){let i=Ee(n);if(this._data=new i(t.length),n==="int64"){let r=this._data;for(let a=0;a<t.length;a++)r[a]=BigInt(Math.round(t[a]??0))}else for(let r=0;r<t.length;r++)this._data[r]=t[r]??0}else this._data=t}get data(){return this.checkDisposed(),this._data}get isDisposed(){return this._isDisposed}checkDisposed(){if(this._isDisposed)throw new x("Cannot access disposed tensor",M.TENSOR_DISPOSED,{tensorId:this.id})}toFloat32Array(){if(this.checkDisposed(),this._data instanceof Float32Array)return this._data;let t=new Float32Array(this.size);for(let e=0;e<this.size;e++)t[e]=Number(this._data[e]??0);return t}toArray(){if(this.checkDisposed(),this.dtype==="int64"){let t=this._data,e=[];for(let n=0;n<t.length;n++)e.push(Number(t[n]));return e}return Array.from(this._data)}clone(){this.checkDisposed();let t=this._data.constructor,e=new t(this._data);return new o(e,this.shape,this.dtype)}dispose(){this._isDisposed||(this._isDisposed=!0,Object.assign(this,{_data:null}))}get(...t){if(this.checkDisposed(),t.length!==this.shape.length)throw new x(`Expected ${this.shape.length} indices, got ${t.length}`,M.INVALID_ARGUMENT,{expectedIndices:this.shape.length,gotIndices:t.length});let e=0,n=1;for(let s=this.shape.length-1;s>=0;s--){let i=t[s]??0,r=this.shape[s]??1;if(i<0||i>=r)throw new x(`Index ${i} out of bounds for dimension ${s} with size ${r}`,M.INVALID_ARGUMENT,{index:i,dimension:s,size:r});e+=i*n,n*=r}return Number(this._data[e]??0)}set(t,...e){if(this.checkDisposed(),e.length!==this.shape.length)throw new x(`Expected ${this.shape.length} indices, got ${e.length}`,M.INVALID_ARGUMENT,{expectedIndices:this.shape.length,gotIndices:e.length});let n=0,s=1;for(let i=this.shape.length-1;i>=0;i--){let r=e[i]??0,a=this.shape[i]??1;if(r<0||r>=a)throw new x(`Index ${r} out of bounds for dimension ${i} with size ${a}`,M.INVALID_ARGUMENT,{index:r,dimension:i,size:a});n+=r*s,s*=a}this._data[n]=t}reshape(t){this.checkDisposed();let e=ee(t);if(e!==this.size)throw new x(`Cannot reshape tensor of size ${this.size} to shape ${JSON.stringify(t)} (size ${e})`,M.TENSOR_SHAPE_MISMATCH,{currentSize:this.size,newSize:e,newShape:t});let n=this._data.constructor,s=new n(this._data);return new o(s,t,this.dtype)}transpose(){if(this.checkDisposed(),this.shape.length!==2)throw new x("Transpose is currently only supported for 2D tensors",M.NOT_IMPLEMENTED,{shape:this.shape});let[t,e]=this.shape,n=new Float32Array(this.size);for(let s=0;s<t;s++)for(let i=0;i<e;i++)n[i*t+s]=Number(this._data[s*e+i]??0);return new o(n,[e,t],this.dtype)}toString(){return`Tensor(shape=[${this.shape.join(", ")}], dtype=${this.dtype})`}};function $t(o,t,e="float32"){if(Array.isArray(o)&&o.length>0&&Array.isArray(o[0])){let s=o.length,i=o[0].length,r=[];for(let a of o){if(a.length!==i)throw new x("Nested arrays must have consistent dimensions",M.INVALID_ARGUMENT);r.push(...a)}return new g(r,t??[s,i],e)}let n=t??[o.length];return new g(o,n,e)}function nt(o,t="float32"){let e=ee(o),n=Ee(t),s=new n(e);return new g(s,o,t)}function Bt(o,t="float32"){let e=ee(o),n=Ee(t),s=new n(e);return s.fill(1),new g(s,o,t)}function Ut(o,t,e="float32"){let n=ee(o),s=Ee(e),i=new s(n);return i.fill(t),new g(i,o,e)}function qt(o,t="float32"){let e=ee(o),n=new Float32Array(e);for(let s=0;s<e;s++)n[s]=Math.random();return new g(n,o,t)}function jt(o,t="float32"){let e=ee(o),n=new Float32Array(e);for(let s=0;s<e;s+=2){let i=Math.random(),r=Math.random(),a=Math.sqrt(-2*Math.log(i)),c=2*Math.PI*r;n[s]=a*Math.cos(c),s+1<e&&(n[s+1]=a*Math.sin(c))}return new g(n,o,t)}function Wt(o,t,e=1,n="float32"){t===void 0&&(t=o,o=0);let s=Math.ceil((t-o)/e),i=new Float32Array(s);for(let r=0;r<s;r++)i[r]=o+r*e;return new g(i,[s],n)}function Ht(o,t,e=50,n="float32"){let s=new Float32Array(e),i=(t-o)/(e-1);for(let r=0;r<e;r++)s[r]=o+r*i;return new g(s,[e],n)}function Vt(o,t="float32"){let e=new Float32Array(o*o);for(let n=0;n<o;n++)e[n*o+n]=1;return new g(e,[o,o],t)}function Gt(o,t){if(typeof t=="number"){let i=new Float32Array(o.size),r=o.toFloat32Array();for(let a=0;a<o.size;a++)i[a]=(r[a]??0)+t;return new g(i,o.shape,o.dtype)}if(o.size!==t.size)throw new x("Tensor sizes must match for element-wise operations",M.TENSOR_SHAPE_MISMATCH,{aShape:o.shape,bShape:t.shape});let e=new Float32Array(o.size),n=o.toFloat32Array(),s=t.toFloat32Array();for(let i=0;i<o.size;i++)e[i]=(n[i]??0)+(s[i]??0);return new g(e,o.shape,o.dtype)}function Xt(o,t){if(typeof t=="number"){let i=new Float32Array(o.size),r=o.toFloat32Array();for(let a=0;a<o.size;a++)i[a]=(r[a]??0)-t;return new g(i,o.shape,o.dtype)}if(o.size!==t.size)throw new x("Tensor sizes must match for element-wise operations",M.TENSOR_SHAPE_MISMATCH,{aShape:o.shape,bShape:t.shape});let e=new Float32Array(o.size),n=o.toFloat32Array(),s=t.toFloat32Array();for(let i=0;i<o.size;i++)e[i]=(n[i]??0)-(s[i]??0);return new g(e,o.shape,o.dtype)}function Qt(o,t){if(typeof t=="number"){let i=new Float32Array(o.size),r=o.toFloat32Array();for(let a=0;a<o.size;a++)i[a]=(r[a]??0)*t;return new g(i,o.shape,o.dtype)}if(o.size!==t.size)throw new x("Tensor sizes must match for element-wise operations",M.TENSOR_SHAPE_MISMATCH,{aShape:o.shape,bShape:t.shape});let e=new Float32Array(o.size),n=o.toFloat32Array(),s=t.toFloat32Array();for(let i=0;i<o.size;i++)e[i]=(n[i]??0)*(s[i]??0);return new g(e,o.shape,o.dtype)}function st(o,t){if(typeof t=="number"){let i=new Float32Array(o.size),r=o.toFloat32Array();for(let a=0;a<o.size;a++)i[a]=(r[a]??0)/t;return new g(i,o.shape,o.dtype)}if(o.size!==t.size)throw new x("Tensor sizes must match for element-wise operations",M.TENSOR_SHAPE_MISMATCH,{aShape:o.shape,bShape:t.shape});let e=new Float32Array(o.size),n=o.toFloat32Array(),s=t.toFloat32Array();for(let i=0;i<o.size;i++)e[i]=(n[i]??0)/(s[i]??0);return new g(e,o.shape,o.dtype)}function Kt(o,t){if(o.shape.length!==2||t.shape.length!==2)throw new x("matmul requires 2D tensors",M.INVALID_ARGUMENT,{aShape:o.shape,bShape:t.shape});let[e,n]=o.shape,[s,i]=t.shape;if(n!==s)throw new x(`Matrix dimensions incompatible for multiplication: (${e}x${n}) @ (${s}x${i})`,M.TENSOR_SHAPE_MISMATCH,{aShape:o.shape,bShape:t.shape});let r=new Float32Array(e*i),a=o.toFloat32Array(),c=t.toFloat32Array();for(let l=0;l<e;l++)for(let u=0;u<i;u++){let d=0;for(let h=0;h<n;h++)d+=(a[l*n+h]??0)*(c[h*i+u]??0);r[l*i+u]=d}return new g(r,[e,i],o.dtype)}function F(o,t=-1){let e=o.toFloat32Array(),n=new Float32Array(o.size),s=t<0?o.shape.length+t:t;if(s<0||s>=o.shape.length)throw new x(`Invalid axis ${t} for tensor with ${o.shape.length} dimensions`,M.INVALID_ARGUMENT,{axis:t,shape:o.shape});if(o.shape.length===1){let i=-1/0;for(let a=0;a<o.size;a++)(e[a]??0)>i&&(i=e[a]??0);let r=0;for(let a=0;a<o.size;a++)n[a]=Math.exp((e[a]??0)-i),r+=n[a]??0;for(let a=0;a<o.size;a++)n[a]=(n[a]??0)/r;return new g(n,o.shape,o.dtype)}if(o.shape.length===2&&s===1){let[i,r]=o.shape;for(let a=0;a<i;a++){let c=-1/0;for(let u=0;u<r;u++)(e[a*r+u]??0)>c&&(c=e[a*r+u]??0);let l=0;for(let u=0;u<r;u++)n[a*r+u]=Math.exp((e[a*r+u]??0)-c),l+=n[a*r+u]??0;for(let u=0;u<r;u++)n[a*r+u]=(n[a*r+u]??0)/l}return new g(n,o.shape,o.dtype)}throw new x("Softmax currently only supports 1D tensors or 2D tensors along the last axis",M.NOT_IMPLEMENTED,{shape:o.shape,axis:t})}function Pe(o){let t=o.toFloat32Array(),e=new Float32Array(o.size);for(let n=0;n<o.size;n++)e[n]=Math.max(0,t[n]??0);return new g(e,o.shape,o.dtype)}function De(o){let t=o.toFloat32Array(),e=new Float32Array(o.size);for(let n=0;n<o.size;n++)e[n]=1/(1+Math.exp(-(t[n]??0)));return new g(e,o.shape,o.dtype)}function Jt(o){let t=o.toFloat32Array(),e=new Float32Array(o.size);for(let n=0;n<o.size;n++)e[n]=Math.tanh(t[n]??0);return new g(e,o.shape,o.dtype)}function ve(o,t){let e=o.toFloat32Array();if(t===void 0){let i=0;for(let r=0;r<o.size;r++)i+=e[r]??0;return i}let n=t<0?o.shape.length+t:t;if(n<0||n>=o.shape.length)throw new x(`Invalid axis ${t} for tensor with ${o.shape.length} dimensions`,M.INVALID_ARGUMENT,{axis:t,shape:o.shape});let s=[...o.shape];if(s.splice(n,1),s.length===0){let i=0;for(let r=0;r<o.size;r++)i+=e[r]??0;return i}if(o.shape.length===2){let[i,r]=o.shape;if(n===0){let a=new Float32Array(r);for(let c=0;c<r;c++)for(let l=0;l<i;l++)a[c]=(a[c]??0)+(e[l*r+c]??0);return new g(a,[r],o.dtype)}else{let a=new Float32Array(i);for(let c=0;c<i;c++)for(let l=0;l<r;l++)a[c]=(a[c]??0)+(e[c*r+l]??0);return new g(a,[i],o.dtype)}}throw new x("Sum along axis currently only supports up to 2D tensors",M.NOT_IMPLEMENTED,{shape:o.shape,axis:t})}function Yt(o,t){if(t===void 0)return ve(o)/o.size;let e=ve(o,t);if(typeof e=="number")return e/(o.shape[t]??1);let n=o.shape[t]??1;return st(e,n)}function Zt(o,t){let e=o.toFloat32Array();if(t===void 0){let s=0,i=e[0]??-1/0;for(let r=1;r<o.size;r++)(e[r]??-1/0)>i&&(i=e[r]??-1/0,s=r);return s}let n=t<0?o.shape.length+t:t;if(o.shape.length===2&&n===1){let[s,i]=o.shape,r=new Float32Array(s);for(let a=0;a<s;a++){let c=0,l=e[a*i]??-1/0;for(let u=1;u<i;u++)(e[a*i+u]??-1/0)>l&&(l=e[a*i+u]??-1/0,c=u);r[a]=c}return new g(r,[s],"int32")}throw new x("Argmax along axis currently only supports 2D tensors along the last axis",M.NOT_IMPLEMENTED,{shape:o.shape,axis:t})}function en(o,t=0){if(o.length===0)throw new x("Cannot concatenate empty array of tensors",M.INVALID_ARGUMENT);if(o.length===1)return o[0]?.clone()??nt([0]);let e=o[0];if(!e)throw new x("First tensor is undefined",M.INVALID_ARGUMENT);let n=t<0?e.shape.length+t:t;for(let r=1;r<o.length;r++){let a=o[r];if(a){if(a.shape.length!==e.shape.length)throw new x("All tensors must have the same number of dimensions",M.TENSOR_SHAPE_MISMATCH);for(let c=0;c<e.shape.length;c++)if(c!==n&&e.shape[c]!==a.shape[c])throw new x(`Shape mismatch at dimension ${c}`,M.TENSOR_SHAPE_MISMATCH)}}let s=[...e.shape],i=0;for(let r of o)r&&(i+=r.shape[n]??0);if(s[n]=i,e.shape.length===1){let r=new Float32Array(i),a=0;for(let c of o)c&&(r.set(c.toFloat32Array(),a),a+=c.size);return new g(r,s,e.dtype)}throw new x("Concatenation currently only supports 1D tensors",M.NOT_IMPLEMENTED)}var rt=class{constructor(t,e,n,s){f(this,"id");f(this,"modelId");f(this,"priority");f(this,"createdAt");f(this,"_status","pending");f(this,"_startedAt");f(this,"_completedAt");f(this,"_result");f(this,"_error");f(this,"_executor");f(this,"_resolvers",[]);f(this,"_cancelled",!1);this.id=t,this.modelId=e,this.priority=n,this.createdAt=Date.now(),this._executor=s}get status(){return this._status}get startedAt(){return this._startedAt}get completedAt(){return this._completedAt}get result(){return this._result}get error(){return this._error}cancel(){if(this._status==="pending"){this._cancelled=!0,this._status="cancelled",this._completedAt=Date.now();let t=new x("Task was cancelled",M.INFERENCE_CANCELLED,{taskId:this.id});for(let{reject:e}of this._resolvers)e(t);this._resolvers=[]}}wait(){return this._status==="completed"?Promise.resolve(this._result):this._status==="failed"?Promise.reject(this._error):this._status==="cancelled"?Promise.reject(new x("Task was cancelled",M.INFERENCE_CANCELLED,{taskId:this.id})):new Promise((t,e)=>{this._resolvers.push({resolve:t,reject:e})})}async execute(){if(!this._cancelled){this._status="running",this._startedAt=Date.now();try{this._result=await this._executor(),this._status="completed",this._completedAt=Date.now();for(let{resolve:t}of this._resolvers)t(this._result)}catch(t){this._error=t instanceof Error?t:new Error(String(t)),this._status="failed",this._completedAt=Date.now();for(let{reject:e}of this._resolvers)e(this._error)}this._resolvers=[]}}},tn={critical:0,high:1,normal:2,low:3},ot=class{constructor(){f(this,"items",[])}get length(){return this.items.length}isEmpty(){return this.items.length===0}enqueue(t){let e=!1;for(let n=0;n<this.items.length;n++){let s=this.items[n];if(s&&tn[t.priority]<tn[s.priority]){this.items.splice(n,0,t),e=!0;break}}e||this.items.push(t)}dequeue(){return this.items.shift()}peek(){return this.items[0]}remove(t){let e=this.items.findIndex(n=>n.id===t);if(e!==-1){let[n]=this.items.splice(e,1);return n}}getAll(){return[...this.items]}clear(){this.items=[]}};var rs=0;function os(){return`task_${++rs}_${Date.now().toString(36)}`}var is={maxConcurrentTasks:4,maxConcurrentPerModel:1,defaultTimeout:3e4,enableBatching:!1,maxBatchSize:32,batchTimeout:50},de=class{constructor(t={}){f(this,"options");f(this,"queues",new Map);f(this,"runningTasks",new Map);f(this,"allTasks",new Map);f(this,"batchers",new Map);f(this,"listeners",new Map);f(this,"globalRunningCount",0);f(this,"isProcessing",!1);f(this,"disposed",!1);this.options={...is,...t}}getQueue(t){let e=this.queues.get(t);return e||(e=new ot,this.queues.set(t,e)),e}getRunningSet(t){let e=this.runningTasks.get(t);return e||(e=new Set,this.runningTasks.set(t,e)),e}canStartTask(t){if(this.globalRunningCount>=this.options.maxConcurrentTasks)return!1;let e=this.runningTasks.get(t);return!(e&&e.size>=this.options.maxConcurrentPerModel)}async processQueue(){if(this.isProcessing||this.disposed)return;this.isProcessing=!0;try{let e=[];for(let[n,s]of this.queues)for(;!s.isEmpty()&&this.canStartTask(n);){let i=s.dequeue();i&&i.status==="pending"&&(e.push(i),this.getRunningSet(n).add(i.id),this.globalRunningCount++)}await Promise.all(e.map(async n=>{this.emit("inference:start",{taskId:n.id,modelId:n.modelId});try{await n.execute(),this.emit("inference:complete",{taskId:n.id,modelId:n.modelId,duration:(n.completedAt??0)-(n.startedAt??0)})}catch(s){this.emit("inference:error",{taskId:n.id,modelId:n.modelId,error:s})}finally{let s=this.runningTasks.get(n.modelId);s&&s.delete(n.id),this.globalRunningCount--}}))}finally{this.isProcessing=!1}let t=!1;for(let e of this.queues.values())if(!e.isEmpty()){t=!0;break}t&&setTimeout(()=>this.processQueue(),0)}schedule(t,e,n="normal"){if(this.disposed)throw new x("Scheduler has been disposed",M.RUNTIME_NOT_INITIALIZED);let s=new rt(os(),t,n,e);return this.allTasks.set(s.id,s),this.getQueue(t).enqueue(s),this.processQueue(),s}scheduleWithTimeout(t,e,n=this.options.defaultTimeout,s="normal"){let i=()=>new Promise((r,a)=>{let c=setTimeout(()=>{a(new x(`Task timed out after ${n}ms`,M.INFERENCE_TIMEOUT,{timeout:n}))},n);e().then(l=>{clearTimeout(c),r(l)}).catch(l=>{clearTimeout(c),a(l)})});return this.schedule(t,i,s)}async scheduleAll(t){let e=t.map(({modelId:n,executor:s,priority:i})=>this.schedule(n,s,i));return Promise.all(e.map(n=>n.wait()))}getTask(t){return this.allTasks.get(t)}cancelTask(t){let e=this.allTasks.get(t);if(e&&e.status==="pending"){e.cancel();for(let n of this.queues.values())n.remove(t);return!0}return!1}cancelAllForModel(t){let e=this.queues.get(t);if(!e)return 0;let n=0;for(let s of e.getAll())s.status==="pending"&&(s.cancel(),n++);return e.clear(),n}getStats(){let t={totalTasks:this.allTasks.size,pendingTasks:0,runningTasks:0,completedTasks:0,failedTasks:0,cancelledTasks:0,queuedByModel:{}};for(let e of this.allTasks.values())switch(e.status){case"pending":t.pendingTasks++;break;case"running":t.runningTasks++;break;case"completed":t.completedTasks++;break;case"failed":t.failedTasks++;break;case"cancelled":t.cancelledTasks++;break}for(let[e,n]of this.queues)t.queuedByModel[e]=n.length;return t}on(t,e){let n=this.listeners.get(t);n||(n=new Set,this.listeners.set(t,n)),n.add(e)}off(t,e){let n=this.listeners.get(t);n&&n.delete(e)}emit(t,e){let n={type:t,timestamp:Date.now(),data:e},s=this.listeners.get(t);if(s)for(let i of s)try{i(n)}catch(r){console.error("Error in event listener:",r)}}clearHistory(){for(let[t,e]of this.allTasks)(e.status==="completed"||e.status==="failed"||e.status==="cancelled")&&this.allTasks.delete(t)}dispose(){this.disposed=!0;for(let t of this.queues.values()){for(let e of t.getAll())e.cancel();t.clear()}for(let t of this.batchers.values())t.clear();this.queues.clear(),this.runningTasks.clear(),this.allTasks.clear(),this.batchers.clear(),this.listeners.clear()}},ie=null;function Fe(){return ie||(ie=new de),ie}function nn(o){ie&&ie.dispose(),ie=o}function as(o){nn(new de(o))}var cs={initialSize:67108864,maxSize:536870912,growthFactor:1.5,autoGC:!0,gcThreshold:.8},_=class _{constructor(t={}){f(this,"config");f(this,"resources",new Map);f(this,"disposers",new Map);f(this,"listeners",new Map);f(this,"allocated",0);f(this,"peak",0);f(this,"gcScheduled",!1);f(this,"disposed",!1);this.config={...cs,...t}}static getInstance(){return _.instance||(_.instance=new _),_.instance}static configure(t){_.instance&&console.warn("MemoryManager already initialized, configuration may not apply"),_.instance=new _(t)}track(t,e){if(this.disposed)return;let n=this.estimateTensorSize(t);this.resources.set(t.id,{id:t.id,type:"tensor",size:n,createdAt:Date.now(),stackTrace:this.captureStackTrace()}),e&&this.disposers.set(t.id,e),this.allocated+=n,this.peak=Math.max(this.peak,this.allocated),this.checkMemoryThreshold()}trackModel(t,e){if(this.disposed)return;let n=t.metadata.sizeBytes;this.resources.set(t.id,{id:t.id,type:"model",size:n,createdAt:Date.now(),stackTrace:this.captureStackTrace()}),e&&this.disposers.set(t.id,e),this.allocated+=n,this.peak=Math.max(this.peak,this.allocated),this.checkMemoryThreshold()}untrack(t){let e=this.resources.get(t);e&&(this.allocated-=e.size,this.resources.delete(t),this.disposers.delete(t))}release(t){let e=typeof t=="string"?t:t.id,n=this.disposers.get(e);if(n)try{n()}catch(s){console.error("Error disposing resource:",s)}this.untrack(e)}estimateTensorSize(t){let e=this.getBytesPerElement(t.dtype);return t.size*e}getBytesPerElement(t){switch(t){case"float32":return 4;case"float16":return 2;case"int32":return 4;case"int64":return 8;case"uint8":case"int8":case"bool":return 1;default:return 4}}captureStackTrace(){if(typeof Error.captureStackTrace=="function"){let t={};return Error.captureStackTrace(t,this.captureStackTrace),t.stack}return new Error().stack}checkMemoryThreshold(){if(!this.config.autoGC)return;let t=this.allocated/this.config.maxSize;t>=this.config.gcThreshold&&!this.gcScheduled&&(this.gcScheduled=!0,this.emit("memory:warning",{allocated:this.allocated,maxSize:this.config.maxSize,usage:t}),setTimeout(()=>{this.gc(),this.gcScheduled=!1},0))}gc(){this.emit("memory:gc",{before:this.allocated});let t=Date.now(),e=[];for(let[n,s]of this.resources)t-s.createdAt>5*60*1e3&&e.push(n);this.emit("memory:gc",{after:this.allocated,potentialCleanup:e.length})}getStats(){let t=0,e=0;for(let n of this.resources.values())n.type==="tensor"?t++:e++;return{allocated:this.allocated,used:this.allocated,peak:this.peak,tensorCount:t,modelCount:e}}getResourceDetails(){return Array.from(this.resources.values())}detectLeaks(t=10*60*1e3){let e=Date.now(),n=[];for(let s of this.resources.values())e-s.createdAt>t&&n.push(s);return n}on(t,e){let n=this.listeners.get(t);n||(n=new Set,this.listeners.set(t,n)),n.add(e)}off(t,e){let n=this.listeners.get(t);n&&n.delete(e)}emit(t,e){let n={type:t,timestamp:Date.now(),data:e},s=this.listeners.get(t);if(s)for(let i of s)try{i(n)}catch(r){console.error("Error in event listener:",r)}}resetStats(){this.peak=this.allocated}disposeAll(){for(let t of this.resources.keys())this.release(t)}dispose(){this.disposeAll(),this.disposed=!0,this.listeners.clear(),_.instance=null}};f(_,"instance",null);var te=_,he=class o{constructor(t){f(this,"resources",[]);f(this,"children",[]);f(this,"parent",null);t&&(this.parent=t,t.children.push(this))}track(t){return this.resources.push(t),t}createChild(){return new o(this)}keep(t){let e=this.resources.indexOf(t);return e!==-1&&this.resources.splice(e,1),t}dispose(){for(let t of this.children)t.dispose();this.children=[];for(let t=this.resources.length-1;t>=0;t--)try{this.resources[t]?.dispose()}catch(e){console.error("Error disposing resource in scope:",e)}if(this.resources=[],this.parent){let t=this.parent.children.indexOf(this);t!==-1&&this.parent.children.splice(t,1),this.parent=null}}};async function ls(o){let t=new he;try{return await o(t)}finally{t.dispose()}}function us(o){let t=new he;try{return o(t)}finally{t.dispose()}}var fe=class{constructor(t={}){f(this,"maxSize");f(this,"maxModels");f(this,"cache",new Map);f(this,"currentSize",0);this.maxSize=t.maxSize??256*1024*1024,this.maxModels=t.maxModels??5}get(t){let e=this.cache.get(t);if(e)return e.lastAccess=Date.now(),e.model}set(t,e){let n=e.metadata.sizeBytes;for(;(this.currentSize+n>this.maxSize||this.cache.size>=this.maxModels)&&this.cache.size>0;)this.evictLRU();this.cache.set(t,{model:e,size:n,lastAccess:Date.now()}),this.currentSize+=n}delete(t){let e=this.cache.get(t);return e?(e.model.dispose(),this.currentSize-=e.size,this.cache.delete(t),!0):!1}has(t){return this.cache.has(t)}evictLRU(){let t=null,e=1/0;for(let[n,s]of this.cache)s.lastAccess<e&&(e=s.lastAccess,t=n);t&&this.delete(t)}clear(){for(let t of this.cache.values())t.model.dispose();this.cache.clear(),this.currentSize=0}getStats(){return{size:this.currentSize,count:this.cache.size,maxSize:this.maxSize,maxModels:this.maxModels}}};function C(){return te.getInstance()}function ds(){return te.getInstance().getStats()}function hs(o){te.getInstance().release(o)}function fs(){te.getInstance().gc()}var _e=new Map,V=new Map,yt=["webgpu","webnn","wasm"],ne=class ne{constructor(){f(this,"listeners",new Map);f(this,"defaultRuntime","auto")}static getInstance(){return ne.instance||(ne.instance=new ne),ne.instance}register(t,e){_e.set(t,e)}async getRuntime(t="auto"){if(t==="auto")return this.getBestRuntime();let e=V.get(t);if(e)return e;let n=_e.get(t);if(!n)throw new x(`Runtime '${t}' is not registered`,M.RUNTIME_NOT_AVAILABLE,{runtime:t});if(e=n(),!await e.isAvailable())throw new x(`Runtime '${t}' is not available in this environment`,M.RUNTIME_NOT_AVAILABLE,{runtime:t});try{await e.initialize()}catch(i){throw new x(`Failed to initialize runtime '${t}': ${i instanceof Error?i.message:String(i)}`,M.RUNTIME_INIT_FAILED,{runtime:t,error:i})}return V.set(t,e),this.emit("runtime:ready",{runtime:t}),e}async getBestRuntime(){for(let t of yt)try{let e=V.get(t);if(e)return e;let n=_e.get(t);if(!n)continue;let s=n();if(await s.isAvailable())return await s.initialize(),V.set(t,s),this.emit("runtime:ready",{runtime:t}),s}catch{continue}throw new x("No runtime available. Please ensure WebGPU, WebNN, or WASM is supported.",M.RUNTIME_NOT_AVAILABLE,{triedRuntimes:yt})}async detectAvailableRuntimes(){let t=new Map;for(let e of yt){let n=_e.get(e);if(!n){t.set(e,!1);continue}try{let s=n();t.set(e,await s.isAvailable())}catch{t.set(e,!1)}}return t}async getCapabilities(t){return(await this.getRuntime(t)).capabilities}setDefaultRuntime(t){this.defaultRuntime=t}getDefaultRuntimeType(){return this.defaultRuntime}disposeRuntime(t){let e=V.get(t);e&&(e.dispose(),V.delete(t))}disposeAll(){for(let[t,e]of V)e.dispose(),V.delete(t)}on(t,e){let n=this.listeners.get(t);n||(n=new Set,this.listeners.set(t,n)),n.add(e)}off(t,e){let n=this.listeners.get(t);n&&n.delete(e)}emit(t,e){let n={type:t,timestamp:Date.now(),data:e},s=this.listeners.get(t);if(s)for(let i of s)try{i(n)}catch(r){console.error("Error in event listener:",r)}}};f(ne,"instance",null);var R=ne,ws=0;function xs(){return`model_${++ws}_${Date.now().toString(36)}`}var O=class{constructor(t,e,n){f(this,"id");f(this,"metadata");f(this,"runtime");f(this,"_isLoaded",!0);f(this,"_dispose");this.id=xs(),this.metadata=t,this.runtime=e,this._dispose=n}get isLoaded(){return this._isLoaded}dispose(){this._isLoaded&&(this._isLoaded=!1,this._dispose(),C().untrack(this.id))}};async function wt(o,t={}){let n=await R.getInstance().getRuntime(t.runtime??"auto"),{loadModelData:s}=await Promise.resolve().then(()=>(Le(),rn)),i=await s(o,{cache:t.cache??!0,resumable:t.resumable??!0,chunkSize:t.chunkSize,forceDownload:t.forceDownload,onProgress:t.onProgress?a=>{t.onProgress(a.percent/100)}:void 0});return await n.loadModel(i,t)}async function xt(o,t={}){return(await R.getInstance().getRuntime(t.runtime??"auto")).loadModel(o,t)}async function se(o,t){if(!o.isLoaded)throw new x("Model has been disposed",M.MODEL_NOT_LOADED,{modelId:o.id});let n=await R.getInstance().getRuntime(o.runtime);return Fe().schedule(o.id,()=>n.run(o,t)).wait()}async function bs(o,t){let e=Fe(),s=await R.getInstance().getRuntime(o.runtime),i=t.map(r=>e.schedule(o.id,()=>s.run(o,r)));return Promise.all(i.map(r=>r.wait()))}function Ts(){return R.getInstance()}function ge(o,t){R.getInstance().register(o,t)}async function Ms(){return R.getInstance().getBestRuntime()}async function ye(){return R.getInstance().detectAvailableRuntimes()}var ae={STORAGE:128,COPY_SRC:4,COPY_DST:8,MAP_READ:1},on={COMPUTE:4},we=class{constructor(){f(this,"name","webgpu");f(this,"adapter",null);f(this,"device",null);f(this,"models",new Map);f(this,"initialized",!1)}get capabilities(){return{concurrency:!0,quantization:!0,float16:!0,dynamicShapes:!1,maxBatchSize:64,availableMemory:this.device?.limits.maxBufferSize??256*1024*1024}}async isAvailable(){if(typeof navigator>"u"||!navigator.gpu)return!1;try{return await navigator.gpu.requestAdapter()!==null}catch{return!1}}async initialize(){if(!this.initialized){if(!navigator.gpu)throw new x("WebGPU is not supported in this browser",M.RUNTIME_NOT_AVAILABLE);if(this.adapter=await navigator.gpu.requestAdapter({powerPreference:"high-performance"}),!this.adapter)throw new x("Failed to get WebGPU adapter",M.RUNTIME_INIT_FAILED);this.device=await this.adapter.requestDevice({requiredFeatures:[],requiredLimits:{}}),this.device.lost.then(t=>{console.error("WebGPU device was lost:",t.message),this.initialized=!1,this.device=null}),this.initialized=!0}}async loadModel(t,e={}){this.ensureInitialized();let n=this.parseModelData(t),s={shaders:new Map,pipelines:new Map,weights:new Map,bindGroupLayouts:[],config:n};await this.uploadWeights(t,s),await this.createPipelines(s);let i=`webgpu_${Date.now().toString(36)}`;this.models.set(i,s);let r={name:n.name||e.metadata?.name||"unknown",version:n.version,inputs:n.inputs.map(c=>({name:c.name,dtype:c.dtype,shape:c.shape})),outputs:n.outputs.map(c=>({name:c.name,dtype:c.dtype,shape:c.shape})),sizeBytes:t.byteLength,quantization:e.quantization??"float32",format:"WebInfer"},a=new O(r,"webgpu",()=>this.unloadModel(i));return C().trackModel(a,()=>a.dispose()),a}async run(t,e){return this.ensureInitialized(),this.executeModel(e,t.metadata)}async executeModel(t,e){let n=this.device,s=[];for(let i of e.outputs){let r=i.shape.reduce((u,d)=>u*d,1),a=n.createBuffer({size:r*4,usage:ae.STORAGE|ae.COPY_SRC}),c=n.createBuffer({size:r*4,usage:ae.MAP_READ|ae.COPY_DST}),l=new Float32Array(r);if(t.length>0&&t[0]){let u=t[0].toFloat32Array();for(let d=0;d<Math.min(r,u.length);d++)l[d]=u[d]??0}s.push(new g(l,i.shape,"float32")),a.destroy(),c.destroy()}return s}parseModelData(t){try{let e=new TextDecoder,n=e.decode(new Uint8Array(t,0,Math.min(1024,t.byteLength)));if(n.trim().startsWith("{")){let s=n.indexOf(`
---
...
\u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2518
  `.trim()}function Dn(o){let t=o.percentFaster>0?"\u2191":o.percentFaster<0?"\u2193":"=",e=o.winner==="comparison"?"Comparison is faster!":o.winner==="baseline"?"Baseline is faster!":"Results are similar";return`
\u250C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2510
...
`)}
  ...
  `}function Xn(o){let t=o.getSummary(),e=o.getSamples(),n=e[e.length-1],s=c=>c<1024?`${c} B`:c<1024*1024?`${(c/1024).toFixed(1)} KB`:c<1024*1024*1024?`${(c/(1024*1024)).toFixed(1)} MB`:`${(c/(1024*1024*1024)).toFixed(1)} GB`,i=(c,l,u=20)=>{let d=Math.round(c/l*u);return"\u2588".repeat(d)+"\u2591".repeat(u-d)},r=["\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557","\u2551             WebInfer.js Performance Monitor Dashboard                   \u2551","\u2560\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2563","\u2551                                                                          \u2551",`\u2551  Total Inferences:  ${t.totalInferences.toString().padStart(10)}                                      \u2551`,`\u2551  Avg Inference:     ${t.avgInferenceTime.toFixed(2).padStart(10)}ms                                     \u2551`,`\u2551  Throughput:        ${t.avgThroughput.toFixed(2).padStart(10)} ops/s                                 \u2551`,`\u2551  Avg FPS:           ${Math.round(t.avgFPS).toString().padStart(10)}                                      \u2551`,"\u2551                                                                          \u2551","\u255F\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2562","\u2551 Memory Usage                                                             \u2551",`\u2551  Heap:    ${i(t.avgMemoryUsage,1)} ${(t.avgMemoryUsage*100).toFixed(0).padStart(3)}%            \u2551`,`\u2551  Used:    ${s(n?.memory.usedHeap??0).padStart(10)}                                          \u2551`,`\u2551  Tensor:  ${s(n?.memory.tensorMemory??0).padStart(10)}                                          \u2551`,`\u2551  Cache:   ${s(n?.memory.cacheMemory??0).padStart(10)}                                          \u2551`,"\u2551                                                                          \u2551","\u255F\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2562","\u2551 Inference Time History (last 30 samples)                                 \u2551","\u2551                                                                          \u2551"],a=e.slice(-30);if(a.length>0){let c=a.map(d=>d.inference.avgTime),l=Math.max(...c,1),u=5;for(let d=u;d>0;d--){let h="\u2551  ";for(let p of c){let m=Math.ceil(p/l*u);h+=m>=d?"\u2593":" "}r.push(h.padEnd(76)+"\u2551")}r.push("\u2551  "+"\u2500".repeat(30)+"                                            \u2551")}return r.push("\u2551                                                                          \u2551"),r.push(`\u2551  Last updated: ${new Date().toLocaleString().padEnd(40)}             \u2551`),r.push("\u255A\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255D"),r.join(`
`)}var et=null;function _t(o){return(!et||o)&&(et=new ze(o)),et}function Qn(o){let t=_t(o);return t.start(),t}function Kn(){et?.stop()}async function Ns(o,t){let e=o instanceof ArrayBuffer?o:await Ct(o),n=e.byteLength,s,i=0,r=0;switch(t.method){case"int8":({data:s,layersQuantized:i,layersSkipped:r}=Rs(e,t));break;case"uint8":({data:s,layersQuantized:i,layersSkipped:r}=Os(e,t));break;case"float16":({data:s,layersQuantized:i,layersSkipped:r}=$s(e,t));break;case"int4":({data:s,layersQuantized:i,layersSkipped:r}=Bs(e,t));break;default:s=e}return{modelData:s,originalSize:n,quantizedSize:s.byteLength,compressionRatio:n/s.byteLength,stats:{layersQuantized:i,layersSkipped:r}}}async function Ct(o){return new ArrayBuffer(0)}function Rs(o,t){let e=new Float32Array(o),n=new Int8Array(e.length),s=0;for(let r=0;r<e.length;r++){let a=Math.abs(e[r]??0);a>s&&(s=a)}let i=s/127;for(let r=0;r<e.length;r++)n[r]=Math.round((e[r]??0)/i);return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Os(o,t){let e=new Float32Array(o),n=new Uint8Array(e.length),s=1/0,i=-1/0;for(let a=0;a<e.length;a++){let c=e[a]??0;c<s&&(s=c),c>i&&(i=c)}let r=(i-s)/255;for(let a=0;a<e.length;a++)n[a]=Math.round(((e[a]??0)-s)/r);return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function $s(o,t){let e=new Float32Array(o),n=new Uint16Array(e.length);for(let s=0;s<e.length;s++)n[s]=Us(e[s]??0);return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Bs(o,t){let e=new Float32Array(o),n=new Uint8Array(Math.ceil(e.length/2)),s=0;for(let r=0;r<e.length;r++){let a=Math.abs(e[r]??0);a>s&&(s=a)}let i=s/7;for(let r=0;r<e.length;r+=2){let a=Math.round((e[r]??0)/i)+8,c=Math.round((e[r+1]??0)/i)+8;n[r/2]=(a&15)<<4|c&15}return{data:n.buffer,layersQuantized:1,layersSkipped:0}}function Us(o){let t=new Float32Array(1),e=new Int32Array(t.buffer);t[0]=o;let n=e[0]??0,s=n>>16&32768,i=n>>12&2047,r=n>>23&255;return r<103?s:r>142?(s|=31744,s|=(r===255?0:1)&&n&8388607,s):r<113?(i|=2048,s|=(i>>114-r)+(i>>113-r&1),s):(s|=r-112<<10|i>>1,s+=i&1,s)}async function qs(o,t){let e=o instanceof ArrayBuffer?o:await Ct(o),n=new Float32Array(e),s=n.length,r=[...n.map(Math.abs)].sort((u,d)=>u-d),a=Math.floor(t.sparsity*r.length),c=r[a]??0,l=0;for(let u=0;u<n.length;u++)Math.abs(n[u]??0)<c&&(n[u]=0,l++);return{modelData:n.buffer,actualSparsity:l/s,parametersPruned:l,totalParameters:s}}async function js(o){let t=o instanceof ArrayBuffer?o.byteLength:o.metadata.sizeBytes,e=Math.floor(t/4);return{totalParameters:e,sizeBytes:t,layers:[],estimatedFlops:e*2,memoryRequirements:{weights:t,activations:t*.1,total:t*1.1}}}async function Ws(o,t={}){let{warmupRuns:e=3,runs:n=10}=t;for(let h=0;h<e;h++)await o();let s=[];for(let h=0;h<n;h++){let p=performance.now();await o(),s.push(performance.now()-p)}let r=s.reduce((h,p)=>h+p,0)/s.length,a=Math.min(...s),c=Math.max(...s),u=s.map(h=>Math.pow(h-r,2)).reduce((h,p)=>h+p,0)/s.length,d=Math.sqrt(u);return{avgTime:r,minTime:a,maxTime:c,stdDev:d,throughput:1e3/r,times:s}}async function Hs(o,t){let e=o instanceof ArrayBuffer?o:await Ct(o);switch(t){case"json":{let n=new Float32Array(e);return JSON.stringify(Array.from(n))}case"binary":case"onnx":default:return e}}async function Fi(){let o=await ye();return Array.from(o.values()).some(t=>t)}async function Li(){let o=await ye();return o.get("webgpu")?"webgpu":o.get("webnn")?"webnn":o.get("wasm")?"wasm":null}async function _i(o){let t=new X;await Promise.all(o.map(async e=>{if(!await t.get(e)){let n=await fetch(e);n.ok&&await t.put(e,n)}}))}var Vs="0.1.0";async function Ci(){let o=await ye();return{version:Vs,runtimes:{webgpu:o.get("webgpu")??!1,webnn:o.get("webnn")??!1,wasm:o.get("wasm")??!1,auto:!0},features:["concurrent-execution","batch-processing","memory-management","model-caching","quantization"]}}export{oe as AudioPreprocessor,v as BasePipeline,ce as Cache,fn as EMOTION_LABELS,M as ErrorCodes,K as FeatureExtractionPipeline,Oe as IMAGENET_LABELS,J as ImageClassificationPipeline,re as ImagePreprocessor,Re as InferenceCache,de as InferenceScheduler,O as LoadedModelImpl,te as MemoryManager,he as MemoryScope,fe as ModelCache,X as ModelDownloadCache,It as POPULAR_MODELS,ze as PerformanceMonitor,R as RuntimeManager,Me as SENTIMENT_LABELS,Q as SentimentAnalysisPipeline,j as TextClassificationPipeline,q as Tokenizer,Vs as VERSION,be as WASMRuntime,we as WebGPURuntime,Se as WebInferDebugger,x as WebInferError,g as WebInferTensor,xe as WebNNRuntime,Gt as add,js as analyzeModel,$n as analyzeModelDetailed,Wt as arange,Zt as argmax,Ws as benchmark,Fn as benchmarkMemory,En as benchmarkSuite,mt as cancelPreload,ht as clearModelCache,vn as compareBenchmarks,en as concat,as as configureScheduler,Wn as createAsciiHistogram,xn as createAudioPreprocessor,le as createBasicTokenizer,un as createCache,yn as createFeatureExtractionPipeline,bn as createImageClassificationPipeline,Be as createImagePreprocessor,Es as createPipelines,gn as createSentimentAnalysisPipeline,Hn as createTensorHeatmap,mn as createTextClassificationPipeline,an as createWASMRuntime,Ce as createWebGPURuntime,Ne as createWebNNRuntime,dt as deleteCachedModel,vt as dequantizeFloat16,Ye as dequantizeInt8,Rn as dequantizeTensor,St as dequantizeUint8,jn as disableDebugging,st as div,je as downloadConfig,We as downloadModel,kt as downloadTokenizer,qn as enableDebugging,Hs as exportModel,Bn as exportModelAdvanced,Vt as eye,zt as float16ToFloat32,Pn as formatBenchmarkResult,Dn as formatComparisonResult,Un as formatTensorInspection,He as fromHub,Ve as fromTask,Ut as full,fs as gc,Xn as generateAsciiDashboard,Gn as generateDashboardHTML,ye as getAvailableRuntimes,Ms as getBestRuntime,Li as getBestRuntimeType,ut as getCachedModel,Lt as getDebugger,At as getDefaultModel,Ci as getInfo,C as getMemoryManager,ds as getMemoryStats,ft as getModelCacheStats,Sn as getModelInfo,_t as getMonitor,hn as getPipelineFactory,pt as getPreloadStatus,gt as getPreloadedModel,Ts as getRuntimeManager,Fe as getScheduler,Ae as inspectTensor,me as isModelCached,Fi as isSupported,Ht as linspace,wt as loadModel,H as loadModelData,xt as loadModelFromBuffer,pn as loadTokenizer,$e as loadTokenizerFromHub,Kt as matmul,Yt as mean,An as modelExists,Qt as mul,Bt as ones,zn as pipeline,_i as preload,ct as preloadModel,lt as preloadModels,wn as preprocessText,qs as prune,On as pruneModel,Dt as pruneTensor,Ns as quantize,Pt as quantizeModel,Nn as quantizeTensor,jt as randn,qt as random,ln as registerAllBackends,$ as registerPipeline,ge as registerRuntime,hs as release,Pe as relu,bs as runBatchInference,Ie as runBenchmark,se as runInference,nn as setScheduler,De as sigmoid,F as softmax,Qn as startMonitoring,Kn as stopMonitoring,Xt as sub,ve as sum,Jt as tanh,$t as tensor,Vn as visualizeModelArchitecture,ls as withMemoryScope,us as withMemoryScopeSync,nt as zeros};
//# sourceMappingURL=WebInfer.browser.min.js.map
