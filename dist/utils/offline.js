/**
 * WebInfer.js - Offline/PWA Support
 *
...
export class OfflineManager {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            serviceWorkerPath: config.serviceWorkerPath ?? '/WebInfer-sw.js',
        };
    }
    /**
    ...
    async registerServiceWorker() {
        ...
        try {
            ...
            console.log('WebInfer.js service worker registered:', registration.scope);
            // Handle updates
                ...
                if (newWorker) {
                    newWorker.onstatechange = () => {
                        if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                            console.log('New WebInfer.js service worker available');
                        }
                    };
                }
            };
    ...
    async openDatabase() {
        return new Promise((resolve, reject) => {
            const request = indexedDB.open('WebInfer-model-cache', 1);
...
export function generateServiceWorker(options = {}) {
    const { cacheName = 'WebInfer-v1', modelUrls = [], cacheFirst = true, } = options;
    return `
// WebInfer.js Service Worker
...
self.addEventListener('install', (event) => {
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then((cache) => {
        console.log('[WebInfer SW] Caching core files');
        return cache.addAll([
          '/',
          '/WebInfer.browser.min.js',
          ...MODEL_URLS,
        ]);
      })
...
async function cacheModel(url) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('[WebInfer SW] Cached model:', url);
    }
  } catch (error) {
    console.error('[WebInfer SW] Failed to cache model:', url, error);
  }
}
  `.trim();
}
 ...
 */
export function generateManifest(options = { name: 'WebInfer.js App' }) {
    return {
        name: options.name,
        short_name: options.shortName ?? options.name,
        description: options.description ?? 'ML-powered application built with WebInfer.js',
        start_url: '/',
        display: 'standalone',
