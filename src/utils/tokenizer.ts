...
interface HFTokenizerJSON {
  ...
  post_processor?: {
    ...
    pair?: Array<
      ...
      | { Sequence: { id: string; type_id: number } }
    >;
    special_tokens?: Record<
      string,
      { id: string; ids: number[]; tokens: string[] }
    >;
    [key: string]: unknown;
  };
  decoder?: {
...
export class Tokenizer {
  ...
  private padTokenId: number | undefined;
  ...
  static async fromJSON(json: HFTokenizerJSON | string): Promise<Tokenizer> {
    ...
    const data =
      typeof json === "string" ? (JSON.parse(json) as HFTokenizerJSON) : json;
    ...
    const inferFromVocab = (tok: string): number | undefined =>
      tokenizer.vocab.get(tok);
  ...
  private tokenize(text: string): string[] {
    ...
    const sortedAddedTokens = Array.from(this.addedTokens.keys()).sort(
  ...
  private convertTokensToIds(tokens: string[]): number[] {
    return tokens.map((token) => {
      ...
      // Return UNK (fall back to 0 if undefined)
  ...
  private postProcess(
    ...
    for (const item of template) {
      if ("SpecialToken" in item) {
        const specialToken =
          this.postProcessor.special_tokens?.[item.SpecialToken.id];
        if (specialToken) {
          result.push(...specialToken.ids);
          typeIds.push(
            ...specialToken.ids.map(() => item.SpecialToken.type_id),
          );
        }
  ...
  encode(text: string, options: TokenizerOptions = {}): TokenizedOutput {
    ...
    if (padding === "max_length" && inputIds.length < maxLength) {
      ...
      inputIds = [
        ...inputIds,
        ...(new Array(padLength).fill(this.padTokenId) as number[]),
      ];
      if (returnAttentionMask) {
        attentionMask = [
          ...attentionMask,
          ...(new Array(padLength).fill(0) as number[]),
        ];
      }
      if (tokenTypeIds) {
        tokenTypeIds = [
