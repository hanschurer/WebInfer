/**
 * WebInfer.js - Token Classification (NER) Pipeline
 *
...
import { loadTokenizerFromHub, Tokenizer } from '../utils/tokenizer.js';
import { BasePipeline, registerPipeline, type PipelineResult } from './base.js';
...
export class TokenClassificationPipeline extends BasePipeline<string, NerEntity[]> {
  private tokenizer: Tokenizer | null = null;
  private id2label: string[] = [];
  private injectedModel: LoadedModel | null = null;
   ...
   */
  static fromLoadedModel(
    loadedModel: LoadedModel,
    tokenizer: Tokenizer,
    id2label: string[],
    config: Omit<PipelineConfig, 'task' | 'model'> = { runtime: loadedModel.runtime, cache: true } as never,
  ...
  override async initialize(): Promise<void> {
    if (this.isReady && this.model && this.tokenizer) return;
    ...
    // currently expects WebInfer's custom GPU model format and will crash on ONNX bytes.
  ...
  override async run(input: string, options?: TokenClassificationOptions): Promise<NerEntity[]> {
    ...
    const tokenTypeIds = new WebInferTensor(
      encoded.tokenTypeIds ?? new Array(encoded.inputIds.length).fill(0),
      [1, encoded.inputIds.length],
      'int64'
    );

    const outputs = await runInference(this.model!, [inputIds, attentionMask, tokenTypeIds]);
    const entities = this.postprocessTokenClassification(
      outputs as WebInferTensor[],
      input,
      encoded.inputIds
    );
    ...
    (outputs as WebInferTensor[]).forEach(t => t.dispose());
  ...
  protected override async preprocess(): Promise<never> {
    throw new Error('TokenClassificationPipeline.preprocess is not used');
  }
  protected override async postprocess(): Promise<never> {
  ...
  private postprocessTokenClassification(
    ...
    const shape = logits.shape as number[];
    ...
    type Tok = { token: string; label: string; prob: number; start: number; end: number };
    const toks: Tok[] = [];
   ...
   */
  private alignOffsets(text: string, inputIds: number[], seqLen: number): Array<[number, number]> {
    // Avoid .fill([0,0]) (shared reference)
    const offsets: Array<[number, number]> = Array.from({ length: seqLen }, () => [0, 0]);
    let cursor = 0;
    ...
    for (let i = 0; i < Math.min(seqLen, inputIds.length); i++) {
      ...
      if (pos === -1) pos = text.toLowerCase().indexOf(piece.toLowerCase(), cursor);
      ...
      if (pos === -1) {
        ...
        if (nextNonSpace >= 0) {
          ...
          if (pos === -1) pos = text.toLowerCase().indexOf(piece.toLowerCase(), from);
        }
      }
...
function mergeBioToEntities(
  toks: Array<{ label: string; prob: number; start: number; end: number }>,
  text: string
): NerEntity[] {
  const entities: NerEntity[] = [];

  let curType: NerEntityType | null = null;
  let curStart = 0;
  let curEnd = 0;
  let curProbSum = 0;
  let curCount = 0;
  ...
  const flush = () => {
    if (!curType || curStart === curEnd) return;
  ...
  const toEntityType = (t: string): NerEntityType | null => {
    if (t === 'PER' || t === 'ORG' || t === 'LOC' || t === 'MISC') return t;
    return null;
  };
