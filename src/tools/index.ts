/**
 * WebInfer.js - Tools and Utilities
 *
...
export async function quantize(
  ...
  // Get model data
  const modelData =
    model instanceof ArrayBuffer ? model : await getModelData(model);
...
export async function analyzeModel(
  ...
  // Simplified analysis
  const size =
    model instanceof ArrayBuffer ? model.byteLength : model.metadata.sizeBytes;
