/**
 * WebInfer.js - Visual Debugging Tools
 * 
...
/**
 * Visual debugger for WebInfer.js
 */
export class WebInferDebugger {
   ...
   */
  inspectTensor(tensor: WebInferTensor, name: string = 'tensor'): TensorInspection {
   ...
   * Add input to trace
   */
  traceInput(traceId: string, tensor: WebInferTensor, name: string): void {
   ...
   * Add output to trace
   */
  traceOutput(traceId: string, tensor: WebInferTensor, name: string): void {
    const trace = this.traces.find(t => t.id === traceId);
    if (!trace) return;
   ...
   */
  recordAllocation(tensor: WebInferTensor): void {
   ...
   */
  recordDeallocation(tensor: WebInferTensor): void {
    if (!this.config.trackPerformance) return;
...
let globalDebugger: WebInferDebugger | null = null;

/**
 * Get or create the global debugger instance
 */
export function getDebugger(config?: DebuggerConfig): WebInferDebugger {
 ...
 * Enable debugging
 */
export function enableDebugging(config?: DebuggerConfig): WebInferDebugger {
 ...
 */
export function createTensorHeatmap(tensor: WebInferTensor, width: number = 40): string {
  const shape = tensor.shape as number[];
