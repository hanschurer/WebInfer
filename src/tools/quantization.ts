...
function quantizeToInt4(
  ...
  for (let i = 0; i < data.length; i += 2) {
    ...
    const q1 = Math.max(
      0,
      Math.min(15, Math.round(val1 / scale + zeroPoint + 8)),
    );
    const q2 = Math.max(
      0,
      Math.min(15, Math.round(val2 / scale + zeroPoint + 8)),
    );
...
export function float16ToFloat32(value: number): number {
  ...
  return (
    (sign === 0 ? 1 : -1) * Math.pow(2, exponent - 15) * (1 + mantissa / 1024)
  );
}
...
function serializeQuantizedModel(model: QuantizedModel): ArrayBuffer {
  ...
  view.setUint32(
    offset,
    ["int8", "uint8", "int4", "float16", "dynamic"].indexOf(
...
export async function quantizeModel(
  ...
  for (let i = 0; i < weights.length; i++) {
    ...
    const scaleValue =
      params.scale instanceof Float32Array
        ? Array.from(params.scale)
        : params.scale;
    const zpValue =
      params.zeroPoint instanceof Int32Array
  ...
  const avgScale =
    scales.length > 0 ? scales.reduce((a, b) => a + b, 0) / scales.length : 1;
...
/**
 * Quantize a single WebInferTensor
 */
...
export async function pruneModel(
  ...
  for (const weight of weights) {
    ...
    const tensor = new WebInferTensor(
