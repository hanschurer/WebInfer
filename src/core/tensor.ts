 ...
 */
export class WebInferTensor implements Tensor {
  readonly id: string;
   ...
   */
  reshape(newShape: Shape): WebInferTensor {
    this.checkDisposed();
...
export function tensor(
  ...
  return new WebInferTensor(data as TypedArray | number[], inferredShape, dtype);
}

/**
 * Create a tensor filled with zeros
 */
export function zeros(shape: Shape, dtype: DataType = 'float32'): WebInferTensor {
 ...
 */
export function ones(shape: Shape, dtype: DataType = 'float32'): WebInferTensor {
  const size = calculateSize(shape);
  const TypedArrayCtor = getTypedArrayConstructor(dtype);
  const data = new TypedArrayCtor(size);
 ...
 */
export function random(shape: Shape, dtype: DataType = 'float32'): WebInferTensor {
  const size = calculateSize(shape);
 ...
 */
export function randn(shape: Shape, dtype: DataType = 'float32'): WebInferTensor {
  const size = calculateSize(shape);
  const data = new Float32Array(size);
 ...
 */
export function eye(n: number, dtype: DataType = 'float32'): WebInferTensor {
 ...
 */
export function add(a: WebInferTensor, b: WebInferTensor | number): WebInferTensor {
  if (typeof b === 'number') {
 ...
 */
export function sub(a: WebInferTensor, b: WebInferTensor | number): WebInferTensor {
  if (typeof b === 'number') {
    const result = new Float32Array(a.size);
    const aData = a.toFloat32Array();
    for (let i = 0; i < a.size; i++) {
 ...
 */
export function mul(a: WebInferTensor, b: WebInferTensor | number): WebInferTensor {
  if (typeof b === 'number') {
    const result = new Float32Array(a.size);
    const aData = a.toFloat32Array();
    for (let i = 0; i < a.size; i++) {
 ...
 */
export function matmul(a: WebInferTensor, b: WebInferTensor): WebInferTensor {
 ...
 * Softmax activation
 */
export function softmax(t: WebInferTensor, axis: number = -1): WebInferTensor {
 ...
 * ReLU activation
 */
export function relu(t: WebInferTensor): WebInferTensor {
  const data = t.toFloat32Array();
  const result = new Float32Array(t.size);
 ...
 * Sigmoid activation
 */
export function sigmoid(t: WebInferTensor): WebInferTensor {
  const data = t.toFloat32Array();
  const result = new Float32Array(t.size);
 ...
 * Tanh activation
 */
export function tanh(t: WebInferTensor): WebInferTensor {
  const data = t.toFloat32Array();
  const result = new Float32Array(t.size);
 ...
 */
export function sum(t: WebInferTensor, axis?: number): WebInferTensor | number {
  const data = t.toFloat32Array();
 ...
 */
export function mean(t: WebInferTensor, axis?: number): WebInferTensor | number {
  if (axis === undefined) {
 ...
 */
export function argmax(t: WebInferTensor, axis?: number): number | WebInferTensor {
  const data = t.toFloat32Array();
 ...
 */
export function concat(tensors: WebInferTensor[], axis: number = 0): WebInferTensor {
  if (tensors.length === 0) {
    throw new WebInferError(
